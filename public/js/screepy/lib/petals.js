...
class Petal {
   constructor(config) {
      ...
      this.el.style.borderRadius = `${randomInt(14, (14 + Math.floor(Math.random() * 10)))}px ${randomInt(1, Math.floor(width / 4))}px`;
   }
}
class BlossomScene {
   constructor(config) {
      ...
      this.windMaxSpeed = config.windMaxSpeed || 10;
   ...
   createPetals() {
      for (let i = 0; i < this.numPetals; i++) {
         ...
         let tmpPetal = new Petal({
            customClass: tmpPetalType.customClass
         });
